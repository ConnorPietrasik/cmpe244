/*--------------------------------------------------*  
 * Program: mini6410_pwmHarry.c;                    * 
 * Modified by: HL;              Date: Feb 10, 2016 *
 * Purpose: To add PWM duty cycle change function   *
 *          by altering CMP compare register.       * 
 *--------------------------------------------------*/ 
...
{
	//Connor Pietrasik 015126007: HW pdf says to modify, but this already has duty
	unsigned long tcon;
	unsigned long tcnt;
...
static long s3c64xx_pwm_ioctl(struct file *filep, unsigned int cmd, unsigned long arg1, arg2)
{
	switch (cmd) {
		case PWM_IOCTL_SET_FREQ:	
			if (arg == 0)
				return -EINVAL;
			PWM_Set_Freq(arg1, arg2);	//Connor Pietrasik 015126007: Didn't change anything, this does duty_cycle too
			break;
